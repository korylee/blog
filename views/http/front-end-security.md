...
DNS 劫持由于涉嫌违法，已经被监管起来，而 http 劫持依然非常盛行。
最有效的办法就是全站 https，将 HTTP 加密，这使得运营商无法获取明文，就无法劫持你的响应内容。

### 中间人攻击

中间人（Man-inter-middle-attack,MTTM）指攻击者与通讯的两端分别创建独立的联系，并交换其收到的数据，是通讯的两端认为他们正在通过一个私密的连接与对方直接通话，但实际整个会话都被攻击者完全控制。在中间人攻击中，攻击者可以拦截通讯双方的通话并插入了新的内容
一般的过程如下：
